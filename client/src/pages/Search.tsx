 ...
 */
function CreateSearchResults(data:any[], setItems : (itemList : any[]) => any, addToAcc : (recipeBlock : RecipeData) => any) {
  ...
  let imDiv = []
    ...
    imDiv.push(
      <div >
        ...
        >Add to Your Recipe Collection</button>
...
export default function SearchPage() {
  ...
  const addToAccount = (recipeBlock : RecipeData) => {devRef.doc(googleUser?.uid).get().then((doc) => {
    const datablock = doc.data()
    if (datablock !== undefined) {
      if (datablock.recipe_list !== undefined) {
          devRef.doc(googleUser?.uid).update({
          recipe_list : firebase.firestore.FieldValue.arrayUnion(recipeBlock)
        })
      } else {
        devRef.doc(googleUser?.uid).update({
